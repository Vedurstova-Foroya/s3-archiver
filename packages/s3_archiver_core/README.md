...
Route names must be unique, source and destination storage identities must differ, and OCI locations require `namespace` and `iam_user_ocid`. Source and destination paths are checked symmetrically: two routes may not share an identical source path on the same source storage, nor an identical destination path on the same destination storage, regardless of their paths on the other side. Nested and sibling paths are allowed on both sides, so `data` and `data/fae` (nested) and `data` and `database` (sibling) coexist; only exact path equality on shared storage is rejected, and trailing-slash differences normalize equal. At build time, when nested source paths share the same source storage, objects are routed by longest-prefix match: the most precise (deepest) route claims its subtree, and the parent route archives everything under its path except keys claimed by a more precise overlapping route.
